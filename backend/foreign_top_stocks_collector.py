...
def export_to_json(stocks):
    """輸出 JSON - 外資+投信前50名買超和賣超"""
    # --- 外資排行 ---
    sorted_foreign = sorted(stocks, key=lambda x: x['foreign_net'], reverse=True)
    top_buy = sorted_foreign[:50]
    top_sell = sorted(stocks, key=lambda x: x['foreign_net'])[:50]

    # --- 投信排行 ---
    stocks_with_trust = [s for s in stocks if s.get('trust_net', 0) != 0]
    sorted_trust = sorted(stocks_with_trust, key=lambda x: x['trust_net'], reverse=True)
    trust_top_buy = sorted_trust[:50]
    trust_top_sell = sorted(stocks_with_trust, key=lambda x: x['trust_net'])[:50]
